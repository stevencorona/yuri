...
int Sql_Connect(Sql* self, const char* user, const char* passwd,
  ...
  if (!mysql_real_connect(&self->handle, host, user, passwd, db,
                          (unsigned int)port, NULL /*unix_socket*/,
                          0 /*clientflag*/)) {
...
int Sql_GetColumnNames(Sql* self, const char* table, char* out_buf,
                       size_t buf_len, char sep) {
  char* data;
  size_t len;
  size_t off = 0;
  ...
  out_buf[off] = '\0';
  while (SQL_SUCCESS == Sql_NextRow(self) &&
         SQL_SUCCESS == Sql_GetData(self, 0, &data, &len)) {
...
int Sql_Ping(Sql* self) {
  if (self && mysql_ping(&self->handle) == 0) return SQL_SUCCESS;
  return SQL_ERROR;
}
...
///
/// @private
static int Sql_P_KeepaliveTimer(uintptr_t* data, uintptr_t* none) {
  Sql* self = (Sql*)data;
...
static int Sql_P_Keepalive(Sql* self) {
  ...
  return timer_insert(ping_interval * 1000, ping_interval * 1000,
                      Sql_P_KeepaliveTimer, (uintptr_t*)self, 0);
}
...
size_t Sql_EscapeString(Sql* self, char* out_to, const char* from) {
  if (self) {
    ...
    return (size_t)mysql_escape_string(out_to, from,
                                       (unsigned long)strlen(from));
  }
}

/// Escapes a string.
size_t Sql_EscapeStringLen(Sql* self, char* out_to, const char* from,
                           size_t from_len) {
  if (self) {
    return (size_t)mysql_real_escape_string(&self->handle, out_to, from,
...
int Sql_NextRow(Sql* self) {
  if (self && self->result) {
    ...
    self->lengths = NULL;
    if (mysql_errno(&self->handle) == 0) return SQL_NO_DATA;
  }
  return SQL_ERROR;
}
...
void Sql_ShowDebug_(Sql* self, const char* debug_file,
                    const unsigned long debug_line) {
  if (self == NULL) {
...
#undef SHOW_DEBUG_TYPE_OF
  ...
  ShowDebug("%stype=%s%s, length=%d%s\n", prefix, sign, type_string, length,
            length_postfix);
}
...
static void SqlStmt_P_ShowDebugTruncatedColumn(SqlStmt* self, size_t i) {
  ...
  ShowDebug("column - %lu\n", (unsigned long)i);
  Sql_P_ShowDebugMysqlFieldInfo("data   - ", field->type,
                                field->flags & UNSIGNED_FLAG,
                                self->column_lengths[i].length, "");
  column = &self->columns[i];
  if (column->buffer_type == MYSQL_TYPE_STRING)
    Sql_P_ShowDebugMysqlFieldInfo("buffer - ", column->buffer_type,
                                  column->is_unsigned, column->buffer_length,
                                  "+1(nul-terminator)");
  else
    Sql_P_ShowDebugMysqlFieldInfo("buffer - ", column->buffer_type,
                                  column->is_unsigned, column->buffer_length,
...
int SqlStmt_BindParam(SqlStmt* self, size_t idx, enum SqlDataType buffer_type,
                      void* buffer, size_t buffer_len) {
  if (self == NULL) {
    return SQL_ERROR;
  }
  ...
  if (idx < self->max_params) {
    return Sql_P_BindSqlDataType(self->params + idx, buffer_type, buffer,
...
int SqlStmt_BindColumn(SqlStmt* self, size_t idx, enum SqlDataType buffer_type,
  ...
  if (buffer_type == SQLDT_STRING || buffer_type == SQLDT_ENUM) {
    if (buffer_len < 1) {
      ShowDebug(
          "SqlStmt_BindColumn: buffer_len(%d) is too small, no room for the "
          "nul-terminator\n",
          buffer_len);
      return SQL_ERROR;
    }
  ...
  if (idx < self->max_columns) {
    ...
    return Sql_P_BindSqlDataType(self->columns + idx, buffer_type, buffer,
                                 buffer_len, &self->column_lengths[idx].length,
                                 out_is_null);
  } else {
    return SQL_SUCCESS;  // out of range - ignore
  }
...
int SqlStmt_NextRow(SqlStmt* self) {
  ...
  // check for errors
  if (err == MYSQL_NO_DATA) return SQL_NO_DATA;
  ...
  if (err == MYSQL_DATA_TRUNCATED) {
    ...
    if (!self->bind_columns) {
      ShowSQL(
          "DB error - data truncated (unknown source, columns are not "
          "bound)\n");
      return SQL_ERROR;
    }
  ...
  for (i = 0; i < cols; ++i) {
    ...
    column = &self->columns[i];
#if !defined(MYSQL_DATA_TRUNCATED)
    // MySQL 4.1/(below?) returns success even if data is truncated, so we test
    // truncation manually [FlavioJS]
    ...
    if (column->buffer_type ==
        MYSQL_TYPE_STRING) {  // clear unused part of the string/enum buffer
                              // (and nul-terminate)
      memset((char*)column->buffer + length, 0,
             column->buffer_length - length + 1);
    } else if (column->buffer_type == MYSQL_TYPE_BLOB &&
               length < column->buffer_length) {  // clear unused part of the
                                                  // blob buffer
...
void SqlStmt_FreeResult(SqlStmt* self) {
  if (self) mysql_stmt_free_result(self->stmt);
}

/// Shows debug information (with statement).
void SqlStmt_ShowDebug_(SqlStmt* self, const char* debug_file,
                        const unsigned long debug_line) {
  if (self == NULL) {
    ShowDebug("at %s:%lu -  self is NULL\n", debug_file, debug_line);
  }
