...
static unsigned int tick(void) {
  ...
  return GetTickCount();
       ...
       defined(_POSIX_MONOTONIC_CLOCK) /* posix compliant */) || \
    (defined(__FreeBSD_cc_version) &&                            \
     __FreeBSD_cc_version >= 500005 /* FreeBSD >= 5.1.0 */)
...
unsigned int gettick_nocache(void) { return tick(); }
...
static void push_timer_heap(int tid) {
  ...
  if (timer_heap_num == 0 ||
    ...
    // move elements
    memmove(&timer_heap[pos + 1], &timer_heap[pos],
            sizeof(int) * (timer_heap_num - pos));
    // save new element
    timer_heap[pos] = tid;
  }
...
/// Returns the value of the smallest non-expired timer (or 1 second if there
/// aren't any).
