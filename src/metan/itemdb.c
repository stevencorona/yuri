...
int itemdb_read(const char *itemdb_file) {
  ...
  if (SQL_ERROR ==
          SqlStmt_Prepare(
              ...
              "`ItmRepairable`, `ItmWisdom`, `ItmCon`, `ItmThrown`, "
              "`ItmMaximumAmount`, `ItmIndestructible`, `ItmTimer`, "
              "`ItmMightRequired`, `ItmMinimumLDamage`, `ItmMaximumLDamage`, "
              "`ItmHealing`, `ItmProtected` FROM `Items`") ||
      ...
      SQL_ERROR == SqlStmt_BindColumn(stmt, 12, SQLDT_INT, &item.look_color, 0,
                                      NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 13, SQLDT_USHORT, &item.icon, 0,
                                      NULL, NULL) ||
      ...
      SQL_ERROR == SqlStmt_BindColumn(stmt, 36, SQLDT_INT, &item.thrown, 0,
                                      NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 37, SQLDT_INT, &item.max_amount, 0,
                                      NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 38, SQLDT_UCHAR, &item.ethereal, 0,
                                      NULL, NULL) ||
      SQL_ERROR ==
          SqlStmt_BindColumn(stmt, 39, SQLDT_UINT, &item.time, 0, NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 40, SQLDT_UINT, &item.mightreq, 0,
                                      NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 41, SQLDT_UINT, &item.minLdam, 0,
                                      NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 42, SQLDT_UINT, &item.maxLdam, 0,
                                      NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 43, SQLDT_INT, &item.healing, 0,
                                      NULL, NULL) ||
      SQL_ERROR == SqlStmt_BindColumn(stmt, 44, SQLDT_INT, &item.protected, 0,
                                      NULL, NULL)) {
    SqlStmt_ShowDebug(stmt);
    SqlStmt_Free(stmt);
    return 0;
