...
int command_killall(USER *sd, char *line, lua_State *state) {
    ...
    for (x = 1; x < fd_max; x++) {
        ...
        if (session[x] && (tmpsd = session[x]->session_data) && !session[x]->eof) {
            ...
            if (!tmpsd->status.gm_level) { session[x]->eof = 1; }
...
int command_job(USER *sd, char *line, lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Character` SET `ChaPthId` = '%u', `ChaMark` = '%u' WHERE `ChaId` = '%u'",
                  sd->status.class, sd->status.mark, sd->status.id)) {
