...
int addmobspawn(lua_State *state) {
    ...
    if (map_isloaded(m)) {
        if (SQL_ERROR == Sql_Query(sql_handle,
                                   "INSERT INTO `Spawns%d` (`SpnMapId`, `SpnX`, `SpnY`, `SpnMobId`, `SpnLastDeath`, `SpnStartTime`, `SpnEndTime`, `SpnMobIdReplace`) VALUES(%d, %d, %d, %d, 0, 25, 25, 0)",
...
int checkonline(lua_State *state) {
    ...
    if (lua_isnumber(state, 1)) {
        if (lua_tonumber(state, 1) > 0) {
            ...
            if (SQL_ERROR ==
                SqlStmt_Prepare(stmt, "SELECT `ChaId` FROM `Character` WHERE `ChaOnline` = '1' AND `ChaId` = '%u'", id)
                || SQL_ERROR == SqlStmt_Execute(stmt)) {
                SqlStmt_ShowDebug(stmt);
                SqlStmt_Free(stmt);
                return 0;
        ...
        if (strcmpi(name, "") == 0) {
            if (SQL_ERROR ==
                SqlStmt_Prepare(stmt, "SELECT `ChaId` FROM `Character` WHERE `ChaOnline` = '1' AND `ChaGMLevel` = '0'")
                || SQL_ERROR == SqlStmt_Execute(stmt)) {
                SqlStmt_ShowDebug(stmt);
                SqlStmt_Free(stmt);
                return 0;
            ...
            if (SQL_ERROR ==
                SqlStmt_Prepare(stmt, "Select `ChaId` FROM `Character` WHERE `ChaOnline` = '1' AND `ChaName` = '%s'",
                                strlwr(name))
                || SQL_ERROR == SqlStmt_Execute(stmt)) {
                SqlStmt_ShowDebug(stmt);
                SqlStmt_Free(stmt);
...
int getwarps(lua_State *state) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `SourceMapId`, `SourceX`, `SourceY`, `DestinationMapId`, `DestinationX`, `DestinationY` FROM `Warps` WHERE `SourceMapId` = '%d'",
                                     m)
        || SQL_ERROR == SqlStmt_Execute(stmt)
...
int sl_listAuction(lua_State *state) {
    ...
    if (SQL_ERROR == Sql_Query(sql_handle,
                               "INSERT INTO `Auctions` (`AuctionChaId`, `AuctionItmId`, `AuctionItmAmount`, `AuctionItmDurability`, `AuctionPrice`, `AuctionItmEngrave`, `AuctionItmTimer`, `AuctionItmCustomIcon`, `AuctionItmCustomIconColor`, `AuctionItmCustomLook`, `AuctionItmCustomLookColor`, `AuctionItmProtected`, `AuctionBidding`) VALUES ('%u','%u','%u','%u','%u','%s','%u','%u','%u','%u','%u','%u','%d')",
                               chaid, itmid, amount, dura, price, engrave, timer, customIcon, customIconC, customLook,
                               customLookC, protected, bidding)) {
        Sql_ShowDebug(sql_handle);
        lua_pushnumber(state, 0);
        return 0;
    }
...
int sl_getAuctions(lua_State *state) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `AuctionId`, `AuctionChaId`, `AuctionItmId`, `AuctionItmAmount`, `AuctionItmDurability`, `AuctionPrice`, `AuctionItmEngrave`, `AuctionItmTimer`, `AuctionItmCustomIcon`, `AuctionItmCustomIconColor`, `AuctionItmCustomLook`, `AuctionItmCustomLookColor`, `AuctionItmProtected`, `AuctionBidding` FROM `Auctions`")
        || SQL_ERROR == SqlStmt_Execute(stmt)
        || SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)
...
int sl_copyPoemToPoetry(lua_State *state) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `BrdTopic`, `BrdPost`, `BrdChaId` FROM `Boards` WHERE `BrdBnmId` = '19' AND `BrdId` = '%d'",
                                     BrdId)
        || SQL_ERROR == SqlStmt_Execute(stmt)
    ...
    if (SQL_SUCCESS == SqlStmt_NextRow(stmt)) {
        ...
        if (SQL_ERROR ==
            SqlStmt_Prepare(stmt, "SELECT MAX(`BrdPosition`) FROM `Boards` WHERE `BrdBnmId` = '10'") // poetry board
            || SQL_ERROR == SqlStmt_Execute(stmt)
        ...
        if (SQL_ERROR == Sql_Query(sql_handle,
                                   "UPDATE `Boards` SET `BrdBnmId` = '10', `BrdChaName` = '%s', `BrdPosition` = '%u', `BrdMonth` = '%d', `BrdDay` = '%d' WHERE `BrdId` = '%u'",
...
int sl_getPoems(lua_State *state) {
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `BrdId`, `BrdTopic`, `BrdChaId` FROM `Boards` WHERE `BrdBnmId` = '19'")
        || SQL_ERROR == SqlStmt_Execute(stmt)
...
int sl_selectBulletinBoard(lua_State *state) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `BoardId` FROM `BoardLocations` WHERE `BoardM` = '%u' AND (`BoardX` BETWEEN '%u' AND '%u') AND `BoardY` = '%u' LIMIT 1",
                                     m, x - 1, x + 1, y)
        || SQL_ERROR == SqlStmt_Execute(stmt)
...
int sl_addtoboard(lua_State *state) {
    ...
    if (SQL_ERROR == Sql_Query(sql_handle,
                               "INSERT INTO `Boards` (`BrdBnmId`, `BrdChaName`, `BrdTopic`, `BrdPost`, `BrdMonth`, `BrdDay`, `BrdPosition`) VALUES ('%u', '%s', '%s', '%s', '%d', '%d', '%u')",
...
int sl_getMapModifiers(lua_State *state) {
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `ModModifier`, `ModValue` FROM `MapModifiers` WHERE `ModMapId` = '%u'", mapid)
        || SQL_ERROR == SqlStmt_Execute(stmt)
...
int sl_addMapModifier(lua_State *state) {
    ...
    if (SQL_ERROR == Sql_Query(sql_handle,
                               "INSERT INTO `MapModifiers` (`ModMapId`, `ModModifier`, `ModValue`) VALUES ('%u', '%s', '%d')",
                               mapid, escape, value)) {
        Sql_ShowDebug(sql_handle);
        lua_pushboolean(state, 0);
        return 0;
...
int sl_removeMapModifier(lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `MapModifiers` WHERE `ModMapId` = '%u' AND `ModModifier` = '%s'", mapid,
                  escape)) {
        Sql_ShowDebug(sql_handle);
        lua_pushboolean(state, 0);
        return 0;
...
int sl_setClanTribute(lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Clans` SET `ClnTribute` = '%u' WHERE `ClnId` = '%i'", clanTribute, clan)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushnumber(state, 1);
    }
...
int sl_addClanTribute(lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Clans` SET `ClnTribute` = '%u' + `ClnTribute` WHERE `ClnId` = '%i'", clanTribute,
                  clan)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushnumber(state, 1);
...
int sl_setClanName(lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Clans` SET `ClnName` = '%s' WHERE `ClnId` = '%i'", clanName, clan)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 1);
    }
...
int sl_setClanBankSlots(lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Clans` SET `ClnBankSlots` = '%i' WHERE `ClnId` = '%i'", clan_bank_slots, clan)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushnumber(state, 1);
    }
...
int sl_getClanRoster(lua_State *state) {
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `ChaId`, `ChaClnRank` FROM `Character` WHERE `ChaClnId` = '%u'", clan)
        || SQL_ERROR == SqlStmt_Execute(stmt)
        || SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)
...
int sl_getPathRoster(lua_State *state) {
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `ChaId`, `ChaPthRank` FROM `Character` WHERE `ChaPthId` = '%i'", path)
        || SQL_ERROR == SqlStmt_Execute(stmt)
        || SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)
        || SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_UINT, &rank, 0, NULL, NULL)) {
        SqlStmt_ShowDebug(stmt);
...
int sl_setOfflinePlayerRegistry(lua_State *state) {
    ...
    if (lua_isnumber(state, 1)) {
        ...
        if (SQL_ERROR ==
            SqlStmt_Prepare(stmt, "SELECT `ChaId` FROM `Character` WHERE `ChaName` = '%s'", lua_tostring(state, 1))
            || SQL_ERROR == SqlStmt_Execute(stmt)
            || SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)) {
            SqlStmt_ShowDebug(stmt);
            SqlStmt_Free(stmt);
...
int sl_removeClanMember(lua_State *state) {
    ...
    if (SQL_ERROR == Sql_Query(sql_handle,
                               "UPDATE `Character` SET `ChaClnId` = '0', `ChaClanTitle` = '', `ChaClnRank` = '0' WHERE `ChaId` = '%u'",
                               id)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 0);
...
int sl_addClanMember(lua_State *state) {
    ...
    if (SQL_ERROR == Sql_Query(sql_handle,
                               "UPDATE `Character` SET `ChaClnId` = '%u', `ChaClanTitle` = '', `ChaClnRank` = '1' WHERE `ChaId` = '%u'",
                               clan, id)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 0);
...
int sl_updateClanMemberRank(lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Character` SET `ChaClnRank` = '%u' WHERE `ChaId` = '%u'", rank, id)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 0);
    }
...
int sl_updateClanMemberTitle(lua_State *state) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Character` SET `ChaClanTitle` = '%s' WHERE `ChaId` = '%u'", title, id)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 0);
    }
...
int sl_removePathMember(lua_State *state) {
    ...
    if (sd != NULL) { // if player online
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "UPDATE `Character` SET `ChaPthId` = '%u', `ChaPthRank` = '0' WHERE `ChaId` = '%u'",
                      sd->status.class, id)) {
            Sql_ShowDebug(sql_handle);
            Sql_FreeResult(sql_handle);
            lua_pushboolean(state, 0);
        ...
        if (SQL_ERROR ==
            SqlStmt_Prepare(stmt, "SELECT `ChaPthId` FROM `Character` WHERE `ChaId` = '%u'", id) // get current path
            || SQL_ERROR == SqlStmt_Execute(stmt)
...
int sl_processKanDonations(lua_State *state) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `game_account_email`, `kan_amount`, `paypal_account_email`, `txn_id` FROM `KanDonations` WHERE `game_account_email` != '' AND `Claimed` = 0 AND `payment_status` = 'Completed'")
        || SQL_ERROR == SqlStmt_Execute(stmt)
        || SQL_ERROR ==
           SqlStmt_BindColumn(stmt, 0, SQLDT_STRING, &game_account_email, sizeof(game_account_email), NULL, NULL)
        || SQL_ERROR == SqlStmt_BindColumn(stmt, 1, SQLDT_STRING, &kan_amount, sizeof(kan_amount), NULL, NULL)
        || SQL_ERROR ==
           SqlStmt_BindColumn(stmt, 2, SQLDT_STRING, &paypal_account_email, sizeof(paypal_account_email), NULL, NULL)
    ...
    for (int i = 0; i < SqlStmt_NumRows(stmt) && SQL_SUCCESS == SqlStmt_NextRow(stmt); i++) {
        ...
        if (SQL_ERROR == Sql_Query(sql_handle,
                                   "UPDATE `Accounts` SET `AccountKanBalance` = AccountKanBalance + '%d' WHERE `AccountEmail` = '%s'",
        ...
        if (SQL_ERROR == Sql_Query(sql_handle,
                                   "UPDATE `KanDonations` SET `Claimed` = '1' WHERE `game_account_email` = '%s' AND `txn_id` = '%s'",
        ...
        if (SQL_ERROR == SqlStmt_Prepare(stmt2,
                                         "SELECT `AccountCharId1`, `AccountCharId2`, `AccountCharId3`, `AccountCharId4`, `AccountCharId5`, `AccountCharId6` FROM `Accounts` WHERE `AccountEmail` = '%s'",
                                         game_account_email)
        ...
        for (int x = 0; x < sizeof(characters); x++) {
            if (characters[x].id > 0) {
                ...
                if (SQL_ERROR ==
                    SqlStmt_Prepare(stmt2, "SELECT `ChaName` FROM `Character` WHERE `ChaId` = '%u'", characters[x].id)
                    || SQL_ERROR == SqlStmt_Execute(stmt2)
                    || SQL_ERROR ==
                       SqlStmt_BindColumn(stmt2, 0, SQLDT_STRING, &characters[x].name, sizeof(characters[x].name), NULL,
                                          NULL)) {
                    SqlStmt_ShowDebug(stmt2);
                    SqlStmt_Free(stmt2);
                    return 1;
                ...
                if (SQL_ERROR == SqlStmt_Prepare(stmt2,
                                                 "SELECT MAX(`MalPosition`) FROM `Mail` WHERE `MalChaNameDestination` = '%s' AND `MalDeleted` = '0'",
                                                 characters[x].name)
                    || SQL_ERROR == SqlStmt_Execute(stmt2)
                ...
                if (SQL_ERROR == Sql_Query(sql_handle,
                                           "INSERT INTO `Mail` (`MalChaName`, `MalChaNameDestination`, `MalSubject`, `MalBody`, `MalPosition`, `MalMonth`, `MalDay`, `MalDeleted`, `MalNew`) VALUES ('SYSTEM', '%s', 'Thanks for your donation', '%s', '%d', '%d', '%d', '0', '1')",
...
int sl_guitext(lua_State *state, USER *sd) {
    ...
    int i = 0;
    if (type == -1 && m ==
                      0) { // This check is so Server Broadcast (by passing -1 into first argument and 0 for mapid) will take priority over a game.
        for (i = 0; i < 65535; i++) {
            if (map_isloaded(i)) {
...
int sl_getSetItems(lua_State *state) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `ItmSetItem1`, `ItmSetItem1Amount`, `ItmSetItem2`, `ItmSetItem2Amount`, `ItmSetItem3`, `ItmSetItem3Amount`, `ItmSetItem4`, `ItmSetItem4Amount`, `ItmSetItem5`, `ItmSetItem5Amount` FROM `ItemSets` WHERE `ItmSetId` = '%u' LIMIT 1",
                                     id)
        || SQL_ERROR == SqlStmt_Execute(stmt)
...
int bll_sendparcel(lua_State *state, struct block_list *bl) {
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `ParPosition` FROM `Parcels` WHERE `ParChaIdDestination` = '%u'", receiver)
        || SQL_ERROR == SqlStmt_Execute(stmt)
    ...
    if (SQL_ERROR == Sql_Query(sql_handle,
                               "INSERT INTO `Parcels` (`ParChaIdDestination`, `ParSender`, `ParItmId`, `ParAmount`, `ParChaIdOwner`, `ParEngrave`, `ParPosition`, `ParNpc`, `ParCustomLook`, `ParCustomLookColor`, `ParCustomIcon`, `ParCustomIconColor`, `ParProtected`, `ParItmDura`) VALUES ('%u', '%u', '%u', '%u', '%u', '%s', '%d', '%d', '%d', '%d', '%d', '%d', '%d', '%u')",
                               receiver, sender, item, amount, owner, escape, newest, npcflag, customLook,
                               customLookColor, customIcon, customIconColor, protected, dura)) {
        Sql_ShowDebug(sql_handle);
        lua_pushboolean(state, 0);
        return 1;
    }
...
int bll_getfriends(lua_State *state, struct block_list *bl) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `FndChaId` FROM `Friends` JOIN `Character` ON `FndChaId` = `ChaId` AND `ChaOnline` = '1' WHERE `FndChaName1` = '%s' OR `FndChaName2` = '%s' OR `FndChaName3` = '%s' OR `FndChaName4` = '%s'"
                                     ...
                                     "OR `FndChaName14` = '%s' OR `FndChaName15` = '%s' OR `FndChaName16` = '%s' OR `FndChaName17` = '%s' OR `FndChaName18` = '%s' OR `FndChaName19` = '%s' OR `FndChaName20` = '%s'",
                                     sd->status.name, sd->status.name, sd->status.name,
                                     sd->status.name, sd->status.name, sd->status.name, sd->status.name,
                                     sd->status.name, sd->status.name, sd->status.name, sd->status.name,
                                     sd->status.name, sd->status.name, sd->status.name, sd->status.name,
...
int mobl_setduration(lua_State *state, MOB *mob) {
    ...
    for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
        ...
        if (mid == id && time <= 0 && mob->da[x].caster_id == caster_id && alreadycast == 1) {
            ...
            map_foreachinarea(clif_sendanimation, mob->bl.m, mob->bl.x, mob->bl.y, AREA, BL_PC, mob->da[x].animation,
                              &mob->bl, -1);
...
int pcl_flushduration(lua_State *state, USER *sd) {
    ...
    for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
        ...
        if (flush == 1) {
            ...
            map_foreachinarea(clif_sendanimation, sd->bl.m, sd->bl.x, sd->bl.y, AREA, BL_PC,
                              sd->status.dura_aether[x].animation, &sd->bl, -1);
...
int pcl_setkillcount(lua_State *state, USER *sd) {
    ...
    for (int x = 0;
         x < MAX_KILLREG; x++) { // this executes if mob could not already be found, makes new entry, sets to values
...
int pcl_setCaptchaKey(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `Character` SET `ChaCaptchaKey` = '%s' WHERE `ChaId` = '%u'", md5,
                               sd->status.id)) {
        SqlStmt_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 0);
...
int pcl_getCaptchaKey(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `ChaCaptchaKey` FROM `Character` WHERE `ChaId` = '%u'", sd->status.id)
        || SQL_ERROR == SqlStmt_Execute(stmt)
...
int pcl_setAccountBan(lua_State *state, USER *sd) {
    ...
    if (accountid == 0) { // not regd
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "UPDATE `Character` SET ChaBanned = '%i' WHERE `ChaName` = '%s'", banned, name)) {
            SqlStmt_ShowDebug(sql_handle);
            Sql_FreeResult(sql_handle);
        }
        ...
        if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                         "SELECT `AccountCharId1`, `AccountCharId2`, `AccountCharId3`, `AccountCharId4`, `AccountCharId5`, `AccountCharId6` FROM `Accounts` WHERE `AccountId` = '%u'",
                                         accountid)
            || SQL_ERROR == SqlStmt_Execute(stmt)
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "UPDATE `Accounts` SET `AccountBanned` = '%i' WHERE `AccountId` = '%u'", banned,
                      accountid)) {
            SqlStmt_ShowDebug(sql_handle);
...
int pcl_setHeroShow(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Character` SET `ChaHeroes` = '%u' WHERE `ChaName` = '%s'", sd->status.heroes,
                  sd->status.name)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        return 1;
...
int pcl_updatePath(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Character` SET `ChaPthId` = '%u', `ChaMark` = '%u' WHERE `ChaId` = '%u'", path,
                  mark, sd->status.id)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        return 1;
...
int pcl_updateCountry(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR == Sql_Query(sql_handle, "UPDATE `Character` SET `ChaNation` = '%u' WHERE `ChaId` = '%u'", country,
                               sd->status.id)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        return 1;
...
int pcl_updateMail(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Mail` SET `MalChaNameDestination` = '%s' WHERE `MalChaNameDestination` = '%s'",
                  sd->status.name, name)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 0);
...
int pcl_addKan(lua_State *state, USER *sd) {
    ...
    if (SqlStmt_NumRows(stmt) == 0) {
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "INSERT INTO `Accounts` (`AccountEmail`, `AccountKanBalance`) VALUES ('%s','%d')",
                      email, kan)) {
            Sql_ShowDebug(sql_handle);
            SqlStmt_Free(stmt);
            FREE(email);
...
int pcl_removeKan(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `AccountKanBalance` FROM `Accounts` WHERE `AccountEmail` = '%s'", email)
        || SQL_ERROR == SqlStmt_Execute(stmt)
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Accounts` SET `AccountKanBalance` = '%d' WHERE `AccountEmail` = '%s'",
                  kanBalance, email)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        FREE(email);
...
"SELECT `AccountKanBalance` FROM `Accounts` WHERE `AccountEmail` = '%s'", email)
|| SQL_ERROR ==
SqlStmt_Execute(stmt)
|| SQL_ERROR ==
SqlStmt_BindColumn(stmt,
0, SQLDT_INT, &kanBalance, 0, NULL, NULL)) {
SqlStmt_ShowDebug(stmt);
SqlStmt_Free(stmt);
FREE(email);
lua_pushnumber(state,
...
int pcl_checkKan(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `kan_amount` FROM `KanDonations` WHERE `account_email` = '%s' AND `Claimed` = 0 LIMIT 1",
                                     email)
        || SQL_ERROR == SqlStmt_Execute(stmt)
...
int pcl_claimKan(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT min(payment_id) FROM `KanDonations` WHERE `account_email` = '%s' AND `kan_amount` = '%i Kan' AND `Claimed` = 0",
                                     email, kan)
        || SQL_ERROR == SqlStmt_Execute(stmt)
    ...
    if (SqlStmt_NumRows(stmt) != 0 && payment_id != 0) {
        ...
        if (SQL_ERROR ==
            Sql_Query(sql_handle, "UPDATE `KanDonations` SET `Claimed` = 1 WHERE `payment_id` = '%i'", payment_id)) {
            Sql_ShowDebug(sql_handle);
            SqlStmt_Free(stmt);
            FREE(email);
            lua_pushboolean(state, 0);
...
int pcl_setKan(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Accounts` SET `AccountKanBalance` = '%d' WHERE `AccountEmail` = '%s'", kan,
                  email)) {
        Sql_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        FREE(email);
...
int pcl_setMiniMapToggle(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "UPDATE `Character` SET `ChaMiniMapToggle` = '%u' WHERE `ChaId` = '%d'", miniMapToggle,
                  sd->status.id)) {
        SqlStmt_ShowDebug(sql_handle);
        return 1;
    }
...
int pcl_setduration(lua_State *state, USER *sd) {
    ...
    for (x = 0; x < MAX_MAGIC_TIMERS; x++) { //MAX_DURATIMER
        ...
        if (mid == id && time <= 0 && sd->status.dura_aether[x].caster_id == caster_id && alreadycast == 1) {
            ...
            return 0;
        } else if (sd->status.dura_aether[x].id == id && sd->status.dura_aether[x].caster_id == caster_id &&
                   sd->status.dura_aether[x].aether > 0 && sd->status.dura_aether[x].duration <= 0) {
            sd->status.dura_aether[x].duration = time;
            clif_send_duration(sd, id, time / 1000, map_id2sd(caster_id));
            return 0;
        } else if (sd->status.dura_aether[x].id == id && sd->status.dura_aether[x].caster_id == caster_id &&
                   (sd->status.dura_aether[x].duration > time || recast == 1) && alreadycast == 1) {
            sd->status.dura_aether[x].duration = time;
            clif_send_duration(sd, id, time / 1000, map_id2sd(caster_id));
            return 0;
        } else if (sd->status.dura_aether[x].id == 0 && sd->status.dura_aether[x].duration == 0 && time != 0 &&
                   alreadycast != 1) {
    ...
    for (x = 0; x < MAX_MAGIC_TIMERS; x++) {
        if (sd->status.dura_aether[x].id == id && time <= 0) {
            ...
            return 0;
                   ...
                   (sd->status.dura_aether[x].aether > time || sd->status.dura_aether[x].duration > 0)) {
            sd->status.dura_aether[x].aether = time;
            clif_send_aether(sd, id, time / 1000);
            return 0;
        } else if (sd->status.dura_aether[x].id == 0 && sd->status.dura_aether[x].aether == 0 && time != 0 &&
                   alreadycast != 1) {
            sd->status.dura_aether[x].id = id;
            sd->status.dura_aether[x].aether = time;
            clif_send_aether(sd, id, time / 1000);
...
int pcl_refreshdurations(lua_State *state, USER *sd) {
    ...
    for (i = 0, x = 1; i < MAX_MAGIC_TIMERS; i++, x++) {
        if (sd->status.dura_aether[i].id > 0 && sd->status.dura_aether[i].duration > 0) {
            clif_send_duration(sd, sd->status.dura_aether[i].id, sd->status.dura_aether[i].duration / 1000,
...
int pcl_removeinventoryitem(lua_State *state, USER *sd) {
    ...
    if (owner == 0) {
        ...
        for (x = 0; x < sd->status.maxinv; x++) {
            ...
            if (sd->status.inventory[x].id == id && sd->status.inventory[x].amount < itemdb_stackamount(id) &&
        ...
        for (x = 0; x < sd->status.maxinv; x++) {
            ...
            if (sd->status.inventory[x].id == id && sd->status.inventory[x].amount < itemdb_stackamount(id) &&
                sd->status.inventory[x].owner == owner && !(strcmpi(sd->status.inventory[x].real_name, engrave))) {
                if (sd->status.inventory[x].amount < amount && sd->status.inventory[x].amount > 0) {
                    amount -= sd->status.inventory[x].amount;
                    pc_delitem(sd, x, sd->status.inventory[x].amount, type);
    ...
    for (x = 0; x < sd->status.maxinv; x++) {
        ...
        if (owner == 0) {
            ...
            if (!dura) {
                ...
                if (sd->status.inventory[x].id == id && sd->status.inventory[x].dura >= dura &&
                    !(strcmpi(sd->status.inventory[x].real_name, engrave)) &&
                    sd->status.inventory[x].customLook == customLook &&
                    sd->status.inventory[x].customLookColor == customLookColor &&
                    sd->status.inventory[x].customIcon == customIcon &&
...
int pcl_addEventXP(lua_State *state, USER *sd) {
    ...
    if (strcmp(eventname, "") != 0) {
        ...
        if (SQL_ERROR ==
            SqlStmt_Prepare(stmt, "SELECT `EventId` FROM `RankingEvents` WHERE `EventName` = '%s'", eventname)
            || SQL_ERROR == SqlStmt_Execute(stmt)
    ...
    if (eventid != -1) {
        ...
        if (SQL_ERROR ==
            SqlStmt_Prepare(stmt, "SELECT `ToDate`, `ToTime` FROM `RankingEvents` WHERE `EventId` = '%i'", eventid)
            || SQL_ERROR == SqlStmt_Execute(stmt)
        ...
        if (SQL_ERROR ==
            SqlStmt_Prepare(stmt, "SELECT * FROM `RankingScores` WHERE `EventId` = '%i' AND `ChaId` = '%i'", eventid,
                            sd->status.id) || SQL_ERROR == SqlStmt_Execute(stmt)) {
            SqlStmt_ShowDebug(stmt);
            SqlStmt_Free(stmt);
            return 0;
        }
        ...
        if (SqlStmt_NumRows(stmt) == 0) {
            if (SQL_ERROR == Sql_Query(sql_handle,
                                       "INSERT INTO `RankingScores` (`EventId`, `ChaId`, `ChaName`, `Score`, `EventClaim`) VALUES ('%i', '%i', '%s', '%i', '2')",
            ...
            if (SQL_ERROR == Sql_Query(sql_handle,
                                       "UPDATE `RankingScores` SET `Score` = `Score` + '%u', `ChaName` = '%s' WHERE `EventId` = '%i' AND `ChaId` = '%d'",
...
int pcl_hasspace(lua_State *state, USER *sd) {
    ...
    if (pc_isinvenspace(sd, id, owner, engrave, customLook, customLookColor, customIcon, customIconColor) >=
        sd->status.maxinv) {
        lua_pushboolean(state, 0);
        return 1;
    }
    ...
    for (x = 0; x < sd->status.maxinv; x++) {
        ...
        if (sd->status.inventory[x].id == id && sd->status.inventory[x].owner == owner && engraved == 0) {
...
int pcl_bankdeposit(lua_State *state, USER *sd) {
    ...
    for (x = 0; x < MAX_BANK_SLOTS; x++) {
        if (sd->status.banks[x].item_id == item && sd->status.banks[x].owner == owner &&
            sd->status.banks[x].time == time && !strcmpi(sd->status.banks[x].real_name, engrave) &&
            sd->status.banks[x].protected == protected && sd->status.banks[x].customIcon == customIcon &&
            sd->status.banks[x].customIconColor == customIconColor && sd->status.banks[x].customLook == customLook &&
...
int pcl_clanbankdeposit(lua_State *state, USER *sd) {
    ...
    for (x = 0; x < 255; x++) {
        if (clan->clanbanks[x].item_id == item && clan->clanbanks[x].owner == owner &&
            clan->clanbanks[x].time == time && !strcmpi(clan->clanbanks[x].real_name, engrave) &&
            clan->clanbanks[x].protected == protected && clan->clanbanks[x].customIcon == customIcon &&
            clan->clanbanks[x].customIconColor == customIconColor && clan->clanbanks[x].customLook == customLook &&
...
int pcl_getparcel(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `ParItmId`, `ParAmount`, `ParChaIdOwner`, `ParEngrave`, `ParSender`, `ParPosition`, `ParNpc`, `ParCustomLook`, `ParCustomLookColor`, `ParCustomIcon`, `ParCustomIconColor`, `ParProtected`, `ParItmDura` FROM `Parcels` WHERE `ParChaIdDestination` = '%u'",
                                     sd->status.id)
        || SQL_ERROR == SqlStmt_Execute(stmt)
        || SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)
...
int pcl_removeparcel(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "DELETE FROM `Parcels` WHERE `ParChaIdDestination` = '%u' AND `ParPosition` = '%d'",
                  sd->status.id, pos)) {
        SqlStmt_ShowDebug(sql_handle);
        Sql_FreeResult(sql_handle);
        lua_pushboolean(state, 0);
...
int pcl_expireitem(lua_State *state, USER *sd) {
    ...
    for (x = 0; x < sd->status.maxinv; x++) {
        ...
        if ((sd->status.inventory[x].time > 0 && sd->status.inventory[x].time < t) ||
            (itemdb_time(sd->status.inventory[x].id) > 0 && itemdb_time(sd->status.inventory[x].id) < t)) {
            sprintf(msg, "Your %s has expired! Please visit the cash shop to purchase another.",
    ...
    for (x = 0; x < 14; x++) {
        ...
        if ((sd->status.equip[x].time > 0 && sd->status.equip[x].time < t) ||
            (itemdb_time(sd->status.equip[x].id) > 0 && itemdb_time(sd->status.equip[x].id) < t)) {
            sprintf(msg, "Your %s has expired! Please visit the cash shop to purchase another.",
...
int pcl_getunknownspells(lua_State *state, USER *sd) {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `SplId` FROM `Spells` WHERE `SplPthId` IN ('0', '%d', '%d') AND `SplMark` <= '%d' AND `SplActive` = '1' AND (`SplAlignment` = '%d' OR `SplAlignment` = '-1')",
...
int pcl_getallclassspells(lua_State *state, USER *sd) { // Used to display all spells in game
    ...
    if (SQL_ERROR ==
        SqlStmt_Prepare(stmt, "SELECT `SplId` FROM `Spells` WHERE `SplActive` = '1' AND `SplPthId` = '%u'", class)
        || SQL_ERROR == SqlStmt_Execute(stmt)
        || SQL_ERROR == SqlStmt_BindColumn(stmt, 0, SQLDT_UINT, &id, 0, NULL, NULL)) {
        SqlStmt_ShowDebug(stmt);
        SqlStmt_Free(stmt);
