...
int magicdb_read() {
    ...
    if (SQL_ERROR == SqlStmt_Prepare(stmt,
                                     "SELECT `SplId`, `SplDescription`, `SplIdentifier`, `SplType`, `SplQuestion`, `SplDispel`, `SplAether`, `SplMute`, `SplPthId`, `SplLevel`, `SplMark`, `SplCanFail`, `SplAlignment`, `SplTicker` FROM `Spells` WHERE `SplActive` = '1'")
