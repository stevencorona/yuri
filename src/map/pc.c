...
int pc_addtocurrent2(struct block_list *bl, va_list ap) {
    ...
    if (fl->data.id == fl2->data.id && fl->data.owner == fl2->data.owner &&
        !(strcmpi(fl->data.real_name, fl2->data.real_name)) && fl->data.customIcon == fl2->data.customIcon &&
        fl->data.customIconColor == fl2->data.customIconColor && fl->data.customLook == fl2->data.customLook &&
        fl->data.customLookColor == fl2->data.customLookColor && !(strcmp(fl->data.note, fl2->data.note)) &&
        fl->data.custom == fl2->data.custom && fl->data.protected == fl2->data.protected) {
        ...
        if (sd->status.dura_aether[x].id > 0) {
            ...
            if (sd->status.dura_aether[x].duration > 0) {
                if (tbl != NULL) {
                    ...
                    if (health > 0 || tbl->type == BL_PC) {
                        sl_doscript_blargs(magicdb_yname(sd->status.dura_aether[x].id), "while_cast_250", 2, &sd->bl,
                                           tbl);
                    }
                } else {
        ...
        if (sd->status.dura_aether[x].id > 0) {
            ...
            if (sd->status.dura_aether[x].duration > 0) {
                if (tbl != NULL) {
                    ...
                    if (health > 0 || tbl->type == BL_PC) {
                        sl_doscript_blargs(magicdb_yname(sd->status.dura_aether[x].id), "while_cast_500", 2, &sd->bl,
                                           tbl);
                    }
                } else {
        ...
        if (sd->status.dura_aether[x].id > 0) {
            ...
            if (sd->status.dura_aether[x].duration > 0) {
                if (tbl != NULL) {
                    ...
                    if (health > 0 || tbl->type == BL_PC) {
                        sl_doscript_blargs(magicdb_yname(sd->status.dura_aether[x].id), "while_cast_1500", 2, &sd->bl,
                                           tbl);
                    }
                } else {
        ...
        if (sd->status.dura_aether[x].id > 0) {
            ...
            if (sd->status.dura_aether[x].duration > 0) {
                if (tbl != NULL) {
                    ...
                    if (health > 0 || tbl->type == BL_PC) {
                        sl_doscript_blargs(magicdb_yname(sd->status.dura_aether[x].id), "while_cast_3000", 2, &sd->bl,
                                           tbl);
                    }
                } else {
...
int pc_requestmp(USER *sd) {
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `MalNew` FROM `Mail` WHERE `MalNew` = 1 AND `MalChaNameDestination` = '%s'",
                  sd->status.name)) {
        Sql_ShowDebug(sql_handle);
        return 0;
    }
    ...
    if (SQL_ERROR ==
        Sql_Query(sql_handle, "SELECT `ParItmId` FROM `Parcels` WHERE `ParChaIdDestination`='%u'", sd->status.id)) {
        Sql_ShowDebug(sql_handle);
        return 0;
    }
...
int pc_givexp(USER *sd, unsigned int exp, unsigned int xprate) {
    ...
    for (bl = map[sd->bl.m].block[bx + by * map[sd->bl.m].bxs]; bl != NULL && stack < 32768; bl = bl->next) {
        tsd = map_id2sd(bl->id);
        if ((bl->type & BL_PC) && bl->x == sd->bl.x && bl->y == sd->bl.y && stack < 32768 &&
    ...
    if (sd->status.state != PC_DIE) {
        ...
        for (x = 0; x < MAX_SPELLS; x++) {
            if (sd->status.skill[x] > 0) {
                ...
                sl_doscript_blargs(magicdb_yname(sd->status.skill[x]), "passive", 1, &sd->bl);
            }
        }
...
int pc_isinvenspace(USER *sd, int id, int owner, char *engrave, unsigned int customLook, unsigned int customLookColor,
    ...
    if (itemdb_maxamount(id) > 0) {
        ...
        for (i = 0; i < sd->status.maxinv; i++) {
            if (sd->status.inventory[i].id == id && itemdb_maxamount(id) > 0 &&
                sd->status.inventory[i].owner == owner && !(strcmpi(sd->status.inventory[i].real_name, engrave)) &&
        ...
        for (i = 0; i < 14; i++) {
            if (sd->status.equip[i].id == id && itemdb_maxamount(id) > 0 && sd->status.inventory[i].owner == owner &&
        ...
        for (i = 0; i < sd->status.maxinv; i++) {
            if (sd->status.inventory[i].id == id && sd->status.inventory[i].amount < itemdb_stackamount(id) &&
                maxamount < itemdb_maxamount(id) && sd->status.inventory[i].owner == owner &&
                !(strcmpi(sd->status.inventory[i].real_name, engrave)) &&
                sd->status.inventory[i].customLook == customLook &&
                sd->status.inventory[i].customLookColor == customLookColor &&
                sd->status.inventory[i].customIcon == customIcon &&
...
int pc_isinvenitemspace(USER *sd, int num, int id, int owner, char *engrave) {
    ...
    if (itemdb_maxamount(id) > 0) {
        ...
        } else if (sd->status.inventory[num].id != id || sd->status.inventory[num].owner != owner ||
...
int pc_additem(USER *sd, struct item *fl) {
    ...
    if ((num = pc_isinvenspace(sd, fl->id, fl->owner, fl->real_name, fl->customLook, fl->customLookColor,
                               fl->customIcon, fl->customIconColor)) >= sd->status.maxinv) {
    ...
    do {
        ...
        num = pc_isinvenspace(sd, fl->id, fl->owner, fl->real_name, fl->customLook, fl->customLookColor, fl->customIcon,
                              fl->customIconColor);
...
int pc_canequipitem(USER *sd, int id) {
    ...
    if (pc_isequip(sd, EQ_WEAP)) {
        if (itemdb_type(itemid) == ITM_SHIELD && itemdb_look(sd->status.equip[EQ_WEAP].id) >= 10000 &&
            itemdb_look(sd->status.equip[EQ_WEAP].id) <= 29999)
            return MAP_ERRITM2H;
    }

    if (pc_isequip(sd, EQ_SHIELD)) {
        if (itemdb_type(itemid) == ITM_WEAP && itemdb_look(itemid) >= 10000 && itemdb_look(itemid) <= 29999)
            return MAP_ERRITM2H;
    }
...
int pc_addtocurrent(struct block_list *bl, va_list ap) {
    ...
    if (fl->data.id == sd->status.inventory[id].id && fl->data.owner == sd->status.inventory[id].owner &&
        ...
        !(strcmp(fl->data.note, sd->status.inventory[id].note)) && fl->data.custom == sd->status.inventory[id].custom &&
...
int pc_diescript(USER *sd) {
    ...
    for (i = 0; i < MAX_MAGIC_TIMERS; i++) {
        ...
        if (id > 0) {
            ...
            map_foreachinarea(clif_sendanimation, sd->bl.m, sd->bl.x, sd->bl.y, AREA, BL_PC,
                              sd->status.dura_aether[i].animation, &sd->bl, -1);
