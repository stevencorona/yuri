...
int clif_send_mob_healthscript(MOB* mob, int damage, int critical) {
	...
	if (!mob->current_vita) {
		...
		if(sd != NULL && mob->summon == 0) {
			...
			if (sd->group_count == 0) {  if(mob->exp) addtokillreg(sd,mob->mobid);
			...
			case BL_NPC:
				...
				if(bl->subtype == FLOOR)Radius = 0;
			   	...
			   	if(nd->bl.m == 0 || CheckProximity(one,two,Radius) == 1) {  //F1NPC
					...
					if (sd->status.karma <= -3.0f && strcmp(nd->name,"f1npc") != 0 && strcmp(nd->name,"totem_npc") != 0) {
						clif_scriptmes(sd, nd->bl.id, "Go away scum!", 0, 0);
						return 0;
					}

					/*if (sd->status.state == 1 && strcmpi(nd->name,"f1npc") != 0 && strcmpi(nd->name,"shaman") != 0 && strcmpi(nd->name,"arena_shop") != 0) {
						clif_scriptmes(sd, nd->bl.id, "Go away scum!", 0, 0);
						return 0;
					}*/
