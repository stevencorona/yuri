...
#define BUFVPRINTF(buf, fmt, args)                                             \
  ...
  if (buf.l_ >= 0 && buf.l_ < SBUF_SIZE) { /* static buffer */                 \
    ...
    ShowDebug(                                                                 \
        "showmsg: dynamic buffer used, increase the static buffer size to %d " \
        "or more.\n",                                                          \
        buf.l_ + 1);                                                           \
  }                                                                            \
  // define BUFVPRINTF
...
//  implemented) printf("\x1b[1;30;47m");	// Bright black (grey) on dim
//  white
//
...
//    columns. If the cursor is already on the top line, this sequence is
//    ignored. \e[A is equivalent to \e[1A.
//
// \033[#B - Cursor Down (CUD)
//    Moves the cursor down by the specified number of lines without changing
//    columns. If the cursor is already on the bottom line, this sequence is
//    ignored. \e[B is equivalent to \e[1B.
//
// \033[#C - Cursor Forward (CUF)
//    Moves the cursor forward by the specified number of columns without
//    changing lines. If the cursor is already in the rightmost column, this
//    sequence is ignored. \e[C is equivalent to \e[1C.
//
// \033[#D - Cursor Backward (CUB)
//    Moves the cursor back by the specified number of columns without changing
//    lines. If the cursor is already in the leftmost column, this sequence is
//    ignored. \e[D is equivalent to \e[1D.
//
...
//    number, the second # specifies the column. If you do not specify a
//    position, the cursor moves to the home position: the upper-left corner of
//    the screen (line 1, column 1).
//
...
//    position is unchanged. (default) \033[1J - Clears the screen from start to
//    cursor. The cursor position is unchanged. \033[2J - Clears the screen and
...
//    is unchanged. (default) \033[1K - Clears all characters from start of line
//    to the cursor position. (including the character at the cursor position).
//    The cursor position is unchanged. \033[2K - Clears all characters of the
...
are lost. \e[L is equivalent to \e[1L. \033[#M DL: Delete Line: The block of #
lines at and below the cursor are deleted; all lines below them move up # lines
to fill in the gap, leaving # blank lines at the bottom of the screen. The
cursor position is unchanged. \e[M is equivalent to \e[1M. \033[#\@ ICH: Insert
CHaracter: The cursor character and all characters to the right of it move right
# columns, leaving behind blank space. The cursor position is unchanged. The
rightmost # characters on the line are lost. \e[\@ is equivalent to \e[1\@.
\033[#P
...
int VFPRINTF(HANDLE handle, const char *fmt, va_list argptr) {
  ...
  */
  ...
  while ((q = strchr(p, 0x1b)) != NULL) {  // find the escape character
    if (0 == WriteConsole(handle, p, (DWORD)(q - p), &written,
                          0))  // write up to the escape
    ...
    if (q[1] != '[') {  // write the escape char (whatever purpose it has)
      ...
      for (;;) {
        if (ISDIGIT(*q)) {  // add number to number array, only accept 2digits,
          ...
          //    cursor position is unchanged. \033[1J - Clears the screen from
          //    start to cursor. The cursor position is unchanged. \033[2J -
          ...
          //    end of the line. \033[1K - Clears all characters from start of
          ...
          // column. The default for both is 1
          ...
          [flaviojs] CONSOLE_SCREEN_BUFFER_INFO info;
          GetConsoleScreenBufferInfo(handle, &info);
          saveposition = info.dwCursorPosition;
          */
        } else if (*q == 'u') {  // \033[u - Restore cursor position (RCP)
          /* XXX Two streams are being used. Disabled to avoid inconsistency
          [flaviojs] SetConsoleCursorPosition(handle, saveposition);
          */
        ...
        if (ISDIGIT(*q)) {
          ...
          continue;
                   ...
                   *q == 'f') {  // \033[#;#H - Cursor Position (CUP)
                                 ...
                                 // Moves the cursor down the indicated # of
                                 // rows, to column 1
        } else if (*q == 'F') {  // \033[#F - Cursor Preceding Line (CPL)
                                 // Moves the cursor up the indicated # of rows,
                                 // to column 1.
        } else if (*q == 'G') {  // \033[#G - Cursor Horizontal Absolute (CHA)
                                 // Moves the cursor to indicated column in
                                 // current row.
        } else if (*q == 'L' || *q == 'M' || *q == '@' ||
                   *q == 'P') {  // not implemented, just skip
        } else {                 // no number nor valid sequencer
...
int _vShowMessage(enum msg_type flag, const char *string, va_list ap) {
  ...
  if ((flag == MSG_INFORMATION && msg_silent & 1) ||
  ...
  switch (flag) {
    ...
    case MSG_SQL:  // Bright Violet (For dumping out anything related with SQL)
                   // <- Actually, this is mostly used for SQL errors with the
                   // database, as successes can as well just be anything else...
                   // [Skotlex]
  ...
  if (strlen(DEBUGLOGPATH) > 0) {
    ...
    if (fp == NULL) {
      FPRINTF(STDERR,
              CL_RED "[ERROR]" CL_RESET ": Could not open '" CL_WHITE
                     "%s" CL_RESET "', access denied.\n",
              DEBUGLOGPATH);
      FFLUSH(STDERR);
    } else {
      fprintf(fp, "%s ", prefix);
