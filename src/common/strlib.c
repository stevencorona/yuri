...
char* jstrescapecpy(char* pt, const char* spt) {
  // copy from here
  // WARNING: Target string pt should be able to hold strlen(spt)*2, as each
  // time a escape character is found, the target's final length increases!
  // [Skotlex]
  int i = 0, j = 0;
...
// Converts one or more consecutive occurences of the delimiters into a single
// space and removes such occurences from the beginning and end of string NOTE:
// make sure the string is not const!!
...
int sv_parse(const char* str, int len, int startoff, char delim, int* out_pos,
             int npos, enum e_svopt opt) {
  int i;
  int count;
  enum {
    START_OF_FIELD,
  ...
  if (delim == '\n' && (opt & (SV_TERMINATE_CRLF | SV_TERMINATE_LF))) {
    ShowError(
        ...
        "SV_TERMINATE_LF or SV_TERMINATE_CRLF.\n");
    return -1;  // error
  }
  if (delim == '\r' && (opt & (SV_TERMINATE_CRLF | SV_TERMINATE_CR))) {
    ShowError(
        "sv_parse: delimiter '\\r' is not compatible with options "
        "SV_TERMINATE_CR or SV_TERMINATE_CRLF.\n");
    return -1;  // error
  }
...
#define IS_TERMINATOR()                                           \
  (((opt & SV_TERMINATE_LF) && str[i] == '\n') ||                 \
   ...
   ((opt & SV_TERMINATE_CRLF) && i + 1 < len && str[i] == '\r' && \
    str[i + 1] == '\n'))
#define IS_C_ESCAPE() ((opt & SV_ESCAPE_C) && str[i] == '\\')
#define SET_FIELD_START() \
  if (npos > count * 2 + 2) out_pos[count * 2 + 2] = i
#define SET_FIELD_END()                                 \
  if (npos > count * 2 + 3) out_pos[count * 2 + 3] = i; \
  ++count
...
/// Extra entries in out_fields are filled with the end of the last field (empty
/// string).
///
/// @param str String to parse
/// @param len Length of the string
...
/// @param opt Options that determine the parsing behaviour
/// @return Number of fields found in the string or -1 if an error occured
int sv_split(char* str, int len, int startoff, char delim, char** out_fields,
             int nfields, enum e_svopt opt) {
  int pos[1024];
  int i;
  int done;
  char* end;
...
size_t sv_escape_c(char* out_dest, const char* src, size_t len,
                   const char* escapes) {
  size_t i;
  size_t j;
      ...
      default:
        if (strchr(escapes, src[i])) {  // escape
          ...
          switch (src[i]) {
            ...
            default:  // to octal
              out_dest[j++] =
                  '0' + ((char)(((unsigned char)src[i] & 0700) >> 6));
              out_dest[j++] =
...
/// Opens and parses a file containing delim-separated columns, feeding them to
/// the specified callback function row by row. Tracks the progress of the
/// operation (current line number, number of successfully processed rows).
/// Returns 'true' if it was able to process the specified file, or 'false' if
...
bool sv_readdb(const char* directory, const char* filename, char delim,
               int mincols, int maxcols, int maxrows,
  ...
  if (maxcols > ARRAYLENGTH(fields) - 1) {
    ShowError(
        "sv_readdb: Insufficient column storage in parser for file \"%s\" "
        "(want %d, have only %d). Increase the capacity in the source code "
        "please.\n",
  ...
  while (fgets(line, sizeof(line), fp)) {
    ...
    if (columns < mincols) {
      ShowError(
          "sv_readdb: Insufficient columns in line %d of \"%s\" (found %d, "
          "need at least %d).\n",
    ...
    if (columns > maxcols) {
      ShowError(
          "sv_readdb: Too many columns in line %d of \"%s\" (found %d, maximum "
          "is %d).\n",
    ...
    if (entries == maxrows) {
      ShowError(
          "sv_readdb: Reached the maximum allowed number of entries (%d) when "
          "parsing file \"%s\".\n",
          maxrows, path);
      break;
    }
  ...
  fclose(fp);
  ShowStatus("Done reading '" CL_WHITE "%d" CL_RESET "' entries in '" CL_WHITE
             "%s" CL_RESET "'.\n",
             entries, path);
...
int StringBuf_AppendStr(StringBuf* self, const char* str) {
  ...
  if (needed >= available) {  // not enough space, expand the buffer (minimum
                              // expansion = 1024)
    int off = (int)(self->ptr_ - self->buf_);
