...
#define DB_COUNTSTAT(token) \
  if (stats.##token != UINT32_MAX) ++stats.##token
...
static void db_rotate_right(DBNode node, DBNode* root) {
  ...
  node->left = y->right;
  if (y->right != 0) y->right->parent = node;
  y->parent = node->parent;
  // link y and node's parent
  if (node == *root) {
    *root = y;  // node was root
...
static void db_free_add(DBMap_impl* db, DBNode node, DBNode* root) {
  ...
  if (!(db->options &
        DB_OPT_DUP_KEY)) {  // Make shure we have a key until the node is freed
    old_key = node->key;
...
static void db_free_remove(DBMap_impl* db, DBNode node) {
  ...
  for (i = 0; i < db->free_count; i++) {
    if (db->free_list[i].node == node) {
      if (i < db->free_count -
                  1)  // copy the last item to where the removed one was
...
static void db_free_unlock(DBMap_impl* db) {
  ...
  if (db->free_lock) return;  // Not last lock
...
void* dbit_obj_next(DBIterator* self, DBKey* out_key) {
  ...
  for (; it->ht_index < HASH_SIZE; ++(it->ht_index)) {
    ...
    if (node == NULL) {  // prepare initial node of this hash
      ...
      if (node == NULL) continue;  // next hash
      fake.right = node;
      node = &fake;
    }
    ...
    while (node) {        // next node
      ...
      if (!node->deleted) {  // found next entry
        it->node = node;
        if (out_key) memcpy(out_key, &node->key, sizeof(DBKey));
        return node->data;
      }
    }
  }
...
void* dbit_obj_remove(DBIterator* self) {
  ...
  if (node && !node->deleted) {
    DBMap_impl* db = it->db;
    if (db->cache == node) db->cache = NULL;
    data = node->data;
...
static void* db_obj_get(DBMap* self, DBKey key) {
  ...
  if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
    ShowError(
        "db_get: Attempted to retrieve non-allowed NULL key for db allocated "
        "at %s:%d\n",
        db->alloc_file, db->alloc_line);
  ...
  if (db->cache && db->cmp(key, db->cache->key, db->maxlen) == 0) {
    ...
    if (db->cache->deleted) {
      ShowDebug(
          "db_get: Cache contains a deleted node. Please report this!!!\n");
      return NULL;
    }
#endif
 ...
 * @see DBMap#vgetall
 */
static unsigned int db_obj_vgetall(DBMap* self, void** buf, unsigned int max,
                                   DBMatcher match, va_list args) {
  DBMap_impl* db = (DBMap_impl*)self;
  unsigned int i;
  DBNode node;
  DBNode parent;
    ...
    while (node) {
      ...
      if (!(node->deleted)) {
        ...
        if (match(node->key, node->data, argscopy) == 0) {
          if (buf && ret < max) buf[ret] = node->data;
          ret++;
        }
        va_end(argscopy);
      }
 ...
 * @see DBMap#getall
 */
static unsigned int db_obj_getall(DBMap* self, void** buf, unsigned int max,
                                  DBMatcher match, ...) {
  va_list args;
  unsigned int ret;
 ...
 * @see DBMap#vensure
 */
static void* db_obj_vensure(DBMap* self, DBKey key, DBCreateData create,
                            va_list args) {
  DBMap_impl* db = (DBMap_impl*)self;
  DBNode node;
  DBNode parent = NULL;
  ...
  if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
    ShowError(
        "db_ensure: Attempted to use non-allowed NULL key for db allocated at "
        "%s:%d\n",
        db->alloc_file, db->alloc_line);
    return NULL;  // nullpo candidate
  }
...
static void* db_obj_put(DBMap* self, DBKey key, void* data) {
  ...
  if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
    ShowError(
        "db_put: Attempted to use non-allowed NULL key for db allocated at "
        "%s:%d\n",
        db->alloc_file, db->alloc_line);
    return NULL;  // nullpo candidate
  }
  if (!(data || db->options & DB_OPT_ALLOW_NULL_DATA)) {
    ShowError(
        "db_put: Attempted to use non-allowed NULL data for db allocated at "
        "%s:%d\n",
        db->alloc_file, db->alloc_line);
    return NULL;  // nullpo candidate
  }
 ...
 * Returns the data of the entry.
 * NOTE: The key (of the database) is released in {@link
 * #db_free_add(DBMap_impl*,DBNode,DBNode *)}.
 * @param self Interface of the database
 * @param key Key that identifies the entry
 * @return The data of the entry or NULL if not found
 * @protected
...
static void* db_obj_remove(DBMap* self, DBKey key) {
  ...
  if (!(db->options & DB_OPT_ALLOW_NULL_KEY) && db_is_key_null(db->type, key)) {
    ShowError(
        "db_remove: Attempted to use non-allowed NULL key for db allocated at "
        "%s:%d\n",
        db->alloc_file, db->alloc_line);
    return NULL;  // nullpo candidate
  }
...
static int db_obj_vclear(DBMap* self, DBApply func, va_list args) {
  ...
  db->cache = NULL;
  for (i = 0; i < HASH_SIZE; i++) {
    // Apply the func and delete in the order: left tree, right tree, current
    // node
    node = db->ht[i];
    db->ht[i] = NULL;
    while (node) {
...
static int db_obj_vdestroy(DBMap* self, DBApply func, va_list args) {
  ...
  if (db->global_lock) {
    ShowError(
        "db_vdestroy: Database is already locked for destruction. Aborting "
  ...
  if (db->free_lock)
    ShowWarning(
        "db_vdestroy: Database is still in use, %u lock(s) left. Continuing "
 ...
 *  db_default_release - Get the default releaser for a type of database with
 ...
 */
DBMap* db_alloc(const char* file, int line, DBType type, DBOptions options,
                unsigned short maxlen) {
  DBMap_impl* db;
  unsigned int i;
 ...
 * @see #db_final(void)
 */
void db_init(void) { DB_COUNTSTAT(db_init); }
...
void db_final(void) {
  ...
  ShowInfo(CL_WHITE "Database types" CL_RESET
           ...
           stats.db_int_alloc, stats.db_int_destroy, stats.db_uint_alloc,
           stats.db_uint_destroy, stats.db_string_alloc,
           stats.db_string_destroy, stats.db_istring_alloc,
  ...
  ShowInfo(CL_WHITE "Database function counters" CL_RESET
           ...
           stats.db_rotate_left, stats.db_rotate_right, stats.db_rebalance,
           stats.db_rebalance_erase, stats.db_is_key_null, stats.db_dup_key,
           stats.db_dup_key_free, stats.db_free_add, stats.db_free_remove,
           stats.db_free_lock, stats.db_free_unlock, stats.db_int_cmp,
           stats.db_uint_cmp, stats.db_string_cmp, stats.db_istring_cmp,
           stats.db_int_hash, stats.db_uint_hash, stats.db_string_hash,
           stats.db_istring_hash, stats.db_release_nothing,
           stats.db_release_key, stats.db_release_data, stats.db_release_both,
           stats.dbit_first, stats.dbit_last, stats.dbit_next, stats.dbit_prev,
           stats.dbit_exists, stats.dbit_remove, stats.dbit_destroy,
           stats.db_iterator, stats.db_get, stats.db_getall, stats.db_vgetall,
           stats.db_ensure, stats.db_vensure, stats.db_put, stats.db_remove,
           stats.db_foreach, stats.db_vforeach, stats.db_clear, stats.db_vclear,
           stats.db_destroy, stats.db_vdestroy, stats.db_size, stats.db_type,
           stats.db_options, stats.db_fix_options, stats.db_default_cmp,
           stats.db_default_hash, stats.db_default_release,
           stats.db_custom_release, stats.db_alloc, stats.db_i2key,
           stats.db_ui2key, stats.db_str2key, stats.db_init, stats.db_final);
#endif /* DB_ENABLE_STATS */
}
